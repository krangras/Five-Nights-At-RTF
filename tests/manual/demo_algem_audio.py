...
pygame.display.set_caption("Five Nights At RTF — Algem Audio Demo")
...
from fnar.gameplay.model import GameModel, BASE_GRAPH  # noqa: E402
...
while running:
    ...
    if phase == "WAIT" and phase_timer >= 60:
        ...
        if switch_timer >= SWITCH_INTERVAL:
            switch_timer = 0
            cam_step = (cam_step + 1) % len(CAM_SEQUENCE)
            cam_idx = CAM_SEQUENCE[cam_step]
            p._switch_camera(cam_idx)
    ...
    if m.tablet_open and not m.tablet_animating:
        ...
        for line in lines:
            ...
            fg = font.render(line, True, (255, 255, 0))
