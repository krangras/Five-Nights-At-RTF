...
class OfficeRendererMixin:
    ...
    def draw(self, model):
        ...
        if (model.server_state == "ON" and (model.hack_active or model.hack_progress > 0)) or getattr(
            model, "post_hack_active", False
        ):
        ...
        if model.show_real_screen and not model.laptop_open:
            ...
            small = pygame.transform.smoothscale(self._laptop_offscreen, (self._lp_out_w, self._lp_out_h))
            ...
            warped_rgb = cv2.warpPerspective(bgr, self._lp_M, (self._lp_out_w, self._lp_out_h))
            warped_alpha = cv2.warpPerspective(alpha_src, self._lp_M, (self._lp_out_w, self._lp_out_h))
            rgb = cv2.cvtColor(warped_rgb, cv2.COLOR_BGR2RGB)
            rgba = np.dstack([rgb, warped_alpha])
            surf = pygame.image.frombuffer(rgba.tobytes(), (self._lp_out_w, self._lp_out_h), "RGBA")
        ...
        if model.tablet_open or model.tablet_animating:
            if model.tablet_animating:
                ...
                if cam_idx == 4 and loc == 4:
                    ...
                    if cam_idx in (8, 9, 10, 11) and (
                        model.algem_state_name == "RETREAT" or self._should_show_directional_vent_leave(model, cam_idx)
                    ):
                ...
                seal_blocks_camera = (
                    seal_state is not None and seal_state.name == "CLOSED" and cam_idx in (8, 9, 10, 11)
                )
                ...
                if self.vent_map_mode:
                    ...
                    tab_ty = self.screen_rect.bottom - self.tabbutton_surf.get_height() - self._tab_button_margin_bottom
                    hint_x = mmx + 8
                    hint_y = tab_ty - 34
        ...
        self.screen.blit(self._ctext(self.font, time_str, (255, 255, 255)), (20, 22))
        self.screen.blit(self._ctext(self.font, night_str, (200, 200, 200)), (20, 54))
        ...
        if model.night_start_ticks > 0:
            ...
            display_hour = 12 if model.hour == 0 else model.hour
            title = self._ctext(self.font, f"Night {model.night}", (200, 200, 200))
        ...
        if model.server_overload or model.server_rebooting:
            ...
            if model.server_overload:
                txt = self._ctext(self.font, "OVERLOAD! OPEN LAPTOP TO REBOOT", color)
            else:
