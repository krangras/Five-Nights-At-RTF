...
| **L — Liskov Substitution** | Искусственного наследования нет: там, где наследование не нужно, используется композиция. Это безопаснее для учебного проекта и не ломает поведение подтипами | `GameModel` содержит `AlgemAI`, `VentSealController`, `HackLogPlayer` |
...
- для всех функций и методов добавлены docstring-блоки с `Args` и `Returns`, чтобы назначение кода было понятно прямо из исходников.
